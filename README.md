...
현재 백엔드는 개발 편의를 위해 앱 시작 시 `Base.metadata.create_all()`로 테이블을 생성합니다.
이 방식은 새 테이블 생성에는 사용할 수 있지만, 이미 만들어진 SQLite DB에 컬럼 변경을 자동 반영하지는 않습니다.

DB 모델에 컬럼이 추가되거나 구조가 바뀐 뒤 로컬에서 오류가 나면, 기존 로컬 SQLite DB 파일을 삭제한 뒤 서버를 다시 실행해 테이블을 새로 생성해야 합니다.
단, 실제 운영/공유 데이터가 들어간 DB 파일은 임의로 삭제하면 안 됩니다.

마이그레이션 도구(Alembic 등) 도입 여부는 팀 논의 후 결정합니다.
...
| GET/POST | /account/profile-image | 프로필 사진 조회·저장 |
| GET/PATCH | /account/notifications | 사용자 알림 설정 조회·저장 |
