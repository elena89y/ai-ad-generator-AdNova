...
_PREAMBLE = {
    "strict": (
        ...
        "제품으로 대체한다. 제품이 컵·머그·병·텀블러·마사지 도구처럼 원래 똑바로 서는 형태라면 "
        "옆으로 눕히거나 뒤집거나 기울여 실루엣을 바꾸지 말고, 그 제품 본래의 서는 방향으로 "
        "세워서 보여준다 — 손잡이·부속을 다리·받침으로 바꾸거나 원본에 없는 받침을 만들지 않는다"
        "(단 위에서 내려다보는 탑뷰가 그 템플릿의 의도인 경우는 예외).\n\n"
    ),
    "standard": (
        "업로드한 사진 속 음식·제품이 주인공이다. 구성 재료·형태·색을 원본 그대로 보존하고, "
        "원본에 없는 재료·문자를 만들지 않는다. 아래 연출의 예시 피사체는 구도 참고일 뿐, "
        "반드시 업로드 피사체로 대체해 연출한다. 업로드 피사체가 컵·머그·병·텀블러·마사지 도구처럼 "
        "원래 똑바로 서는 제품이라면 옆으로 눕히거나 뒤집거나 기울이지 말고 그 제품 본래의 서는 "
        "방향으로 세워서 보여준다(음식이나 위에서 내려다보는 탑뷰 의도는 예외).\n\n"
    ),
    "loose": "",
}
