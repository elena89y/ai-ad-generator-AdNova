...
def select_style(img: Image.Image, has_english: bool) -> str:
...
def render_ts1(img: Image.Image, head_en: str) -> Image.Image:
    """TS-1 대형 배경 레터링 — Anton 한 줄, 피사체가 글자를 가린다(z-order)."""
    im = img.convert("RGB")
    w, h = im.size
    layer = im.copy()
    d = ImageDraw.Draw(layer)
    f = _fit_width(d, head_en, "Anton-Regular.ttf", w * 0.86)
...
def plan_typography(img: Image.Image, product_name: str, copy_headline: str,
    ...
    style = select_style(img, has_english)
    head_kr = (product_name or copy_headline or "").strip()
    sub_kr = (copy_headline or "").strip()
    if sub_kr == head_kr:
        sub_kr = ""
    if style in (TS1_BG_LETTERING, TS2_EDITORIAL_SERIF):
...
def render_typography(image_path: str, out_path: str, product_name: str,
                      copy_headline: str, subject_en: str = "") -> str:
    """조판기 v0 진입점 — 스타일 자동 분기 후 렌더, 실패 시 TS-3b 폴백.

    반환: 사용한 스타일 키 (로그·실험 기록용).
    """
    img = Image.open(image_path)
    plan = plan_typography(img, product_name, copy_headline, subject_en)
    try:
