...
def _cover_slide(path: str, copy: DetailPageCopy, pal, size: tuple[int, int], margin_ratio: float) -> Image.Image:
    ...
    draw.rectangle((0, 0, w, 118), fill=(*_PAPER, 242))
    draw.text((margin, 42), copy.product_name or "ADNOVA SELECT", font=_font(25, True), fill=(24, 24, 24))
    draw.line((w - margin - 120, 59, w - margin, 59), fill=pal["accent"], width=5)
    panel_y = int(h * .69)
    draw.rectangle((0, panel_y, w, h), fill=(*_PAPER, 246))
    if copy.product_name:
        draw.text((margin, panel_y + 45), "SIGNATURE MENU", font=_font(22, True), fill=pal["accent"])
    # 표지 메인 = intro_headline(후킹) — 스토리 슬라이드의 story_title 과 다른 문구(D1 해소).
    font = _fit(copy.intro_headline, w - margin * 2, 68, 40)
    draw.text((margin, panel_y + 91), copy.intro_headline, font=font, fill=(20, 20, 20))
    # 보조 한 줄 = 혜택 불릿 첫 항목(다른 슬라이드 미사용 → 겹침 0). 없으면 생략.
    if copy.benefit_bullets:
        draw.text((margin, panel_y + 186), copy.benefit_bullets[0],
                  font=_fit(copy.benefit_bullets[0], w - margin * 2, 30, 22), fill=(78, 76, 72))
...
def _story_slide(path: str, copy: DetailPageCopy, pal, size: tuple[int, int], margin_ratio: float) -> Image.Image:
    ...
    margin = int(w * margin_ratio)
    draw.text((margin, 90), "02", font=_font(24, True), fill=pal["tint"])
    draw.text((margin, 175), copy.product_name or "오늘의 메뉴",
...
def _detail_slide(
    ...
    draw = ImageDraw.Draw(canvas)
    x = int(w * .51)
    draw.text((x, int(h * .70)), "03 / DETAIL", font=_font(21, True), fill=pal["accent"])
...
def _cta_slide(path: str, copy: DetailPageCopy, pal, size: tuple[int, int], margin_ratio: float) -> Image.Image:
    ...
    draw.text((margin, int(h * .69)), copy.product_name or "SIGNATURE", font=_font(24, True), fill=pal["tint"])
    draw.text((margin, int(h * .75)), copy.cta_title, font=_font(52, True), fill="white")
    draw.rectangle((margin, int(h * .85), margin + 238, int(h * .85) + 68), fill=pal["accent"])
    draw.text((margin + 38, int(h * .85) + 18), copy.cta_label, font=_font(25, True), fill="white")
