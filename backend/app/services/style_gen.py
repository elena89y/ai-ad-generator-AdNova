...
def generate_scene(image_path: str, style_key: str, subject_en: str,
    ...
    if extra_style_en:
        if style_key not in _RECOMPOSE_OK:
            instr = (f"Restyle this photo of the {subject_en or 'dish'} to match this direction: "
                     f"{extra_style_en} — background, lighting, mood and dishware all to that look. "
                     "Keep the food faithful (same pieces, real ingredients, its own sauce); do not "
                     "add or invent any food or prop; crop out unrelated items beside it; keep it "
                     f"fresh and appetizing. Re-plate onto tableware matching {extra_style_en}, never "
                     "a cheaper or plainer dish than the mood calls for. Keep colors true. No text.")
        else:
